...
- `UsageStats::cache_read_tokens` / `cache_creation_tokens` (and the same fields on `StreamEvent::UsageEvent`) — prompt-cache reads/writes reported per call and aggregated per turn. Populated by Anthropic (`cache_read_input_tokens` / `cache_creation_input_tokens`) and OpenAI-compatible (`prompt_tokens_details.cached_tokens`, reads only); `None` elsewhere means "unknown".
